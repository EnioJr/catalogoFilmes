...
class MovieList extends Component {
	...
	search = (type, query) => {
		if (query !== "") {
			if (type === "Titulo") this.props.searchMoviesByTitle(query);
			else this.props.searchMoviesByStar(query);
		} else {
			this.props.getMovies();
		}
	};
	...
	render() {
		// instancia dos filmes que esta no banco , movies = tabela do filmes. (usando redux)
		...
		return (
			<div>
				...
				<Container className="d-flex flex-row flex-wrap mt-3">
					{movies.map(movie => (
						<Card className="col-md-3 mb-3" key={movie._id}>
							<CardImg
								top
								width="100%"
								src={movie.imagem}
				...
				{this.state.modal && this.state.movie && (
					<Modal isOpen={this.state.modal} toggle={this.toggle}>
						...
						<ModalBody>
							<p>
								<span className="font-weight-bold">Ano: </span>
							...
							<p className="font-weight-bold">Autor(a):</p>
							<ul>
